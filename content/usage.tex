...
You will get a PDF file named \filebox{main.pdf} in the project directory after the build is complete.
The default result is exactly this document you are reading.

\subsection{Modify the metadata}

You can modify the metadata in \filebox{preamble/meta.tex}.
...
The content of the paper is organized in the \dirbox{content/} directory.
It is a good habit to create a new file per section and name the file after the section name.

You should \macro{\input} the content files in \file{main.tex} in the order they appear in the paper.

\subsection{Insert a figure}

To insert a figure, first place the figure file in the \dirbox{figures/} directory, and then use \macro{\includegraphics} to include the figure in the content file.

If you are using Inkscape to create your figures, you can place the source files in \dirbox{figures/inkscape/} and run \custommacro{./figures/inkscape/export.sh} to export all the \filebox{.svg} files to \filebox{.png} files, which can be included in the document directly.
