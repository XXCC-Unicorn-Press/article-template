...
\begin{itemize}
  ...
  \item \texttt{some-tips.tex}
  \item \texttt{fermats-last-theorem.tex}
  \item \texttt{portrait-of-shakespeare.png}
\end{itemize}
Here are some examples of bad filenames:
\begin{itemize}
  \item \texttt{section1.tex} (not descriptive, and the index may change)
  \item \texttt{some tips.tex} (contains spaces)
  \item \texttt{fermat's-last-theorem.tex} (contains apostrophe)
  \item \texttt{portrait-of-Shakespeare.png} (contains uppercase letters)
\end{itemize}
...
Suppose you have a bibliography item in your \texttt{references.bib} file like this:
\begin{tcblisting}{listing only}
@book{example-book,
  author = {A Person},
  title = {A Book},
...
This will automatically insert the correct citation index wrapped by brackets (e.g., \texttt{[1]}).
You can also specify some additional data by typing \verb|\cite[...]{example-book}|, and the additional data will be included in the brackets (e.g., \texttt{[1, Section 2.1]}).
...
\begin{itemize}
  \item \verb|\includegraphics|, which specifies the image to insert and its size.
  \item \verb|\caption|, which specifies the caption of the figure.
  \item \verb|\label|, which specifies the label for cross-referencing the figure.
\end{itemize}
Remember to place the \verb|\label| command right after the \verb|\caption| command, so that the cross-reference will display the correct figure number.
